...
- Mouse/controller contextual wheel with pagination for groups of up to 12 companions. Owned companions expose Profile, Work, Follow Routine, Stop, Dismiss, and Follow Player; Follow Routine cancels a manual override and immediately restores the scheduled block without repeating one already completed. Unrecruited NPCs expose Recruit; mature trees, breakable stones, mature grab-crops, and water expose compatible group or named-companion commands. Safe empty ground can move a companion there, dismiss the group, or open the fixed-area work flow. Mouse wheel/Page Up/Page Down and controller shoulder buttons change pages; keyboard, D-pad, and either analog stick move focus.
...
- Per-companion hourly routines in the fifth panel tab: paint twenty cells from
  ...
  which covers the entire current map, or Delimited Area, which opens that map
  for placing a square from 3 × 3 through 41 × 41 tiles. Different activities
  can target different locations; when the active block changes, the host moves
  the NPC to a safe tile in the saved location and remote routine work continues
  even when no farmer is watching that map. Routines can repeat daily or run once
  and include a one-click 06–18 work shift. Painting activates the schedule; a
  missing work-area preset stays paused and retries, and is never silently
  treated as Free Area. Circular presets from older saves remain circular until
  the player replaces them with one of the new choices.
...
- Per-member inventory receives safe forage, caught fish, and normal lumbering/mining drops;
  ...
  GUID identity.
- The Inventory tab now has three direct-transfer panes for the player,
  companion cargo, and a nearby assigned chest. Drag a stack to another pane,
  or select it and then its destination on a controller. Transfers validate the
  exact source stack and exact chest, hold the chest mutex in multiplayer, and
  preserve an uncommitted remainder without duplication. Compact viewports page
  each pane with the mouse wheel, Page Up/Page Down, or controller triggers.
- Per-companion cargo filters control automatic deposits: Store wood, Store
  minerals, and Keep food. Manual drag-and-drop remains explicit and therefore
...
- Optional smart assistance can refill an empty equipped watering can at
  canonical reachable water (farm-only or any safe outdoor source), borrow a
  compatible tool from the owner's inventory when no usable one is equipped,
  and deposit filtered cargo when full or after every completed task. Each
  behavior is independently configurable and host-authoritative.
- The sixth Team tab shows every owned companion's map, current activity,
  relevant tool, watering-can charge, cargo usage, and active block reason. It
  also provides owner-scoped Stop all, Deposit all, and Apply routines commands.
...
- Idle companions now use bounded look/emote/jump/shake animations, react to
  location and weather changes, and occasionally interact with another nearby
  companion. The host applies owner-level fairness and cooldowns so cosmetic
  events never compete with active work or flood multiplayer clients.
...
Combat, shearing, milking, sitting, riding, and custom idle sprite-sheet
playback are not implemented. Generic stackable objects can move through
companion cargo; tools still use the four dedicated equipment slots, while
custom/non-stackable objects whose full state can't be serialized are rejected
from companion cargo instead of being damaged. Player-to-chest transfers move
the original runtime item instance, including tool/custom state. The
chest-assignment side panel is currently mouse-only. Tool enchantments and
fishing-rod bait/tackle remain unsupported in companion equipment. Inert
options and the unfinished Combat skill branch are intentionally hidden instead
of being presented as working features. Multiplayer remains experimental until
the manual co-op checklist is completed; crop harvesting by a farmhand's
companion is conservatively disabled because Stardew Valley's crop API credits
`Game1.player` instead of an explicit owner.
